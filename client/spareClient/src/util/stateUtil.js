...
const changeArrayObjectKey=({key,idx,subKey,value,inputState,setInputState})=>{
    let temp=[]
    inputState[key].forEach((i,index)=>{
        if(index==idx){
...
const changeArrayObjectArray=({key,value,idx,subKey,subIdx,inputState,setInputState})=>{
    let temp=[]
    inputState[key].forEach((i,index)=>{
        if(index==idx){
            let subArray=[]
...
//ลบ phone:["0899214409"]
const deleteArrayObjectArray=({key,idx,subKey,subIdx,inputState,setInputState})=>{
    let temp=[]
    inputState[key].forEach((i,index)=>{
        if(index==idx){
            let subArray=[]
...
const addArrayObjectArray=({key,idx,subKey,inputState,setInputState})=>{
    let temp=[]
    inputState[key].forEach((i,index)=>{
        if(index==idx){
...
const stateUtil={changeKey,changeArray,addArray,deleteArray,
    addArrayObject,deleteArrayObject,changeArrayObjectKey,changeArrayObjectArray,
