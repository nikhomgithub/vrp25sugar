...
const init= async (req,res,next)=>{
    ...
    try{
        ...
        if(result&&req.routeAddress=="/user/init"){
            ...
            const temp=createTableTemplate({tableTemplate,userId,shopId})
    ...
    try{
        ...
        if(Array.isArray(restoreData.table)){
            ...
            if(req.routeAddress=="/shop/backup"){
                const resDelete = await Shop.deleteOne({shopName})           
                ...
                const shopId=_id
                console.log("2")
                const resDelete = await Data.deleteMany({})     
                console.log("3")
...
const deletecustom=async(req,res,next)=>{
    ...
    try{
        ...
        if(req.routeAddress=="/user/deletecustom"){
            const TableTemplate=require('../modals/TableTemplate'); 
            await TableTemplate.deleteOne({id,shopId})
        }
